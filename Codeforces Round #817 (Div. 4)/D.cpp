...
void solve() {
    ...
    for (int k = 1; k <= n; ++k) {
        if (indexR != indR.size() && indexL != indL.size()) {
            if (*(indR.begin() + indexR) < n - *(indL.end() - 1 - indexL) - 1) {
                sum -= *(indR.begin() + indexR);
                sum += n - 1 - *(indR.begin() + indexR);
                indexR++;
            } else {

                sum -= n - *(indL.end() - 1 - indexL) - 1;
                sum += *(indL.end() - 1 - indexL);
                indexL++;
            }
        } else if (indexR != indR.size()) {
            sum -= *(indR.begin() + indexR);
            sum += n - 1 - *(indR.begin() + indexR);
            indexR++;
        } else if (indexL != indL.size()) {
            sum -= n - *(indL.end() - 1 - indexL) - 1;
            sum += *(indL.end() - 1 - indexL);
            indexL++;
        }
